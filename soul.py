...
TOKEN = '5666691064:AAECJ9P05C772TQxhcnek79K72JQ9N-mKDM'
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please contact @IPxKINGYT.*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.reply_to(message, "*DM @IPxKINGYT*", parse_mode='Markdown')
    else:
